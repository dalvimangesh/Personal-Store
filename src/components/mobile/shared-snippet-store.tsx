...
import { Plus, Search, Globe, Tag, Users, Copy, Edit2 } from "lucide-react";
import { Card } from "@/components/ui/card";
import { toast } from "sonner";

export function MobileSharedSnippetStore() {
  const { snippets, searchQuery, setSearchQuery, addSnippet, updateSnippet, deleteSnippet, isLoading } = useSharedSnippets();
  const [selectedSnippet, setSelectedSnippet] = useState<SharedSnippet | null>(null);
  const [isEditorOpen, setIsEditorOpen] = useState(false);

  const handleSave = async (data: { title: string; content: string; tags: string[]; allowedUsers: string[] }) => {
    if (selectedSnippet) {
  ...
  const handleEdit = (snippet: SharedSnippet) => {
    ...
    return (
      <div className="fixed inset-0 z-50 bg-background">
        <SharedSnippetEditor 
          ...
          onDelete={async (id) => {
            if (confirm("Delete this shared snippet?")) {
                        ...
                        <div className="flex items-center gap-1">
                            {snippet.allowedUsers.length > 0 ? (
                                <div className="flex items-center text-[10px] text-muted-foreground bg-muted px-1.5 py-0.5 rounded">
                                    ...
                                    {snippet.allowedUsers.length}
                                </div>
                            ) : (
                                <div className="flex items-center text-[10px] text-green-600 bg-green-500/10 px-1.5 py-0.5 rounded">
              ...
              {snippet.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1 px-1">
                    {snippet.tags.slice(0, 3).map(tag => (
                      <span key={tag} className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-cyan-500/10 text-cyan-500">
